...
const i18n = {
  en: {
    ...
    "page.hero.crumb": "Products",
    "page.hero.eyebrow": "Gear Up",
    "page.hero.title": "ALL <em>GEAR</em>",
    ...
    "empty.text": "Try adjusting your search or filters.",
    "empty.btn": "Clear All Filters",
    "footer.brand.desc":
  ...
  fr: {
    ...
    "page.hero.crumb": "Produits",
    "page.hero.eyebrow": "Équipez-vous",
    "page.hero.title": "TOUT L'<em>ÉQUIPEMENT</em>",
    ...
    "empty.text": "Essayez d'ajuster votre recherche ou vos filtres.",
    "empty.btn": "Effacer les Filtres",
    "footer.brand.desc":
  ...
  ar: {
    ...
    "page.hero.crumb": "المنتجات",
    "page.hero.eyebrow": "جهّز نفسك",
    "page.hero.title": "كل <em>المعدات</em>",
    "toolbar.filters": "التصفية",
    ...
    "empty.text": "حاول تعديل بحثك أو الفلاتر.",
    "empty.btn": "مسح كل الفلاتر",
    "footer.brand.desc":
...
function renderCatNav() {
  ...
  mobileCats.innerHTML = allCategories.map((cat) => {
    ...
    if (subs.length === 0) {
      return `<a href="#" class="m-link" onclick="filterByCategory('${cat.id}');toggleMobileMenu();return false;">${catName(cat)}</a>`;
    }
    return `
...
async function loadData() {
  try {
    ...
    categoryOptions = allCategories
      .map((cat) => ({ value: cat.id, nameEn: cat.nameEn || cat.name || "", nameFr: cat.nameFr || "", nameAr: cat.nameAr || "", count: allProducts.filter((p) => p.categoryIds.includes(cat.id)).length }))
      .filter((opt) => opt.count > 0);

    allSubCategories = (res.subCategories || []).map((s) => ({ ...s, categoryIds: parseIds(s.categoryIds) }));
    ...
    if (subParam) {
      const subOpt = subCategoryOptions.find((o) => o.value === subParam);
      if (subOpt) {
    ...
    if (footerList) {
      footerList.innerHTML = allCategories.slice(0, 6)
...
function buildSidebarHTML() {
  ...
  ${(() => {
    const visibleSubs = state.categories.length
      ? subCategoryOptions.filter((opt) => opt.categoryIds.some((cid) => state.categories.includes(cid)))
      : subCategoryOptions;
    ...
    <div class="filter-body">
      <div class="price-range-wrap">
        <div class="price-inputs">
          <input class="price-input" type="number" id="priceMin" placeholder="Min" value="${state.priceMin}" min="0"/>
          <span class="price-sep">–</span>
          <input class="price-input" type="number" id="priceMax" placeholder="Max" value="${state.priceMax}" min="0"/>
        </div>
        <button class="price-apply" onclick="applyPrice()">${t["filter.apply"]}</button>
      </div>
    </div>
  </div>

  <div class="reset-filters">
    <button class="reset-btn" onclick="resetFilters()">${t["filter.reset"]}</button>
  </div>`;
}
...
function getFiltered() {
  ...
  switch (state.sort) {
    case "price-asc": list.sort((a, b) => getProductPrice(a) - getProductPrice(b)); break;
    case "price-desc": list.sort((a, b) => getProductPrice(b) - getProductPrice(a)); break;
    case "newest": list.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)); break;
    case "name": list.sort((a, b) => prodName(a).localeCompare(prodName(b))); break;
  }
  return list;
}
...
/* ══════════════════════════════════════════════════════════════
   RENDER PRODUCTS — same simplified card as the homepage
══════════════════════════════════════════════════════════════ */
function renderProducts() {
  ...
  document.getElementById("resultCount").innerHTML = `<strong>${total}</strong> <span data-i18n="toolbar.products">${t["toolbar.products"]}</span>`;
  ...
  if (page.length === 0) {
    grid.innerHTML = `
      <div class="empty-state">
        ...
        <h3>${t["empty.title"]}</h3>
        <p>${t["empty.text"]}</p>
        <button onclick="resetFilters()">${t["empty.btn"]}</button>
      </div>`;
  ...
  <div class="product-info">
    ...
    <div class="product-actions">
      ${oos
        ...
        : `<button class="btn-cart-icon" onclick="event.stopPropagation();openAddToCartModal('${p.id}')" aria-label="${t["btn.addcart"]}" title="${t["btn.addcart"]}">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
           ...
           <button class="btn-buy" onclick="event.stopPropagation();buyNow('${p.id}')"><span>${t["btn.buynow"]}</span></button>`
      }
    </div>
  </div>
</article>`;
...
function buyNow(id) { window.location.href = `/product-detail?id=${id}`; }
...
function goPage(p) {
  ...
  state.page = p;
  renderProducts();
  window.scrollTo({ top: document.getElementById("productGrid").offsetTop - 140, behavior: "smooth" });
}
...
function renderChips() {
  ...
  state.categories.forEach((v) => chips.push({
    label: catName(categoryOptions.find((o) => o.value === v)) || v,
    remove: () => { state.categories = state.categories.filter((x) => x !== v); refresh(); },
  }));
  state.subCategories.forEach((v) => chips.push({
  ...
  wrap.innerHTML = chips.map((c, i) => `
    ...
    `<button class="clear-all-btn" onclick="resetFilters()">${t["filter.clearall"]}</button>`;

  window._chipRemovers = chips.map((c) => c.remove);
}
function chipRemove(i) { window._chipRemovers[i](); }
...
function handleOverlayClick(e) {
  if (e.target === document.getElementById("filterOverlay")) closeFilterDrawer();
}
...
function openAddToCartModal(productId) {
  ...
  if (_atcFlavorObjs.length) { _atcRenderFlavorOptions(); flavorSec.style.display = ""; } else { flavorSec.style.display = "none"; }
  const variants = Array.isArray(p.variants) ? p.variants : [];
  const weightSec = document.getElementById("atcWeightSection");
  if (variants.length) {
    document.getElementById("atcWeightOptions").innerHTML = variants.map((v, i) => `<button class="atc-option${i === 0 ? " active" : ""}" onclick="atcPickVariant(this,${i})">${varLabel(v, i)}</button>`).join("");
...
function confirmAddToCart() {
  const p = _atcProduct; if (!p || Number(p.stock) <= 0) return;
